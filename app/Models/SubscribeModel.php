...
final class SubscribeModel extends Model
{
    ...
    {
        return $this->fetchAll('SELECT * FROM subscription_plans WHERE is_active = 1 ORDER BY sort_order ASC');
    }

    public function getPlan(int $id): ?array
    {
        return $this->fetch('SELECT * FROM subscription_plans WHERE id = ? AND is_active = 1', [$id]);
    }

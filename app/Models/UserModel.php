...
final class UserModel extends Model
{
    ...
    {
        $sql = 'SELECT id, name, email, phone, password_hash, role, subscription_status FROM users WHERE email = ? LIMIT 1';
        return $this->fetch($sql, [$email]);
    }

    public function findByPhone(string $phone): ?array
    {
        $sql = 'SELECT id, name, email, phone, password_hash, role, subscription_status FROM users WHERE phone = ? LIMIT 1';
